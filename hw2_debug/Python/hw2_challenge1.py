...
def generateLabeledImage(gray_img: np.ndarray, threshold: float) -> np.ndarray:
    ...
    '''
    '''
    gray_img = Image.open(f"data/{img_list[i]}")
    gray_img = np.array(gray_img.convert('L')) / 255.
...
def hw2_challenge1b():
    ...
    labeled_many_obj_1 = Image.open('outputs/labeled_many_objects_1.png')
    labeled_many_obj_1 = np.array(labeled_many_obj_1)
    orig_img = Image.open('data/many_objects_1.png')
    orig_img = np.array(orig_img.convert('L')) / 255.
    obj_db  = compute2DProperties(orig_img, labeled_many_obj_1)
    np.save('outputs/obj_db_1.npy', obj_db)
    print(obj_db)
    ...
    labeled_many_obj_2 = Image.open('outputs/labeled_many_objects_2.png')
    labeled_many_obj_2 = np.array(labeled_many_obj_2)
    orig_img = Image.open('data/many_objects_2.png')
    orig_img = np.array(orig_img.convert('L')) / 255.
    obj_db  = compute2DProperties(orig_img, labeled_many_obj_2)
    np.save('outputs/obj_db_2.npy', obj_db)
    print(obj_db)
